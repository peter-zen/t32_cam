 ...
 * 音频内外部codec选择
 */
#define SELECT_INTERNAL 0
#define SELECT_EXTERNAL 1
...
typedef enum {
	...
	AUDIO_SAMPLE_RATE_12000	= 12000,	/**< 12KHz采样率 */
...
typedef struct {
	...
	IMPAudioAecChn aecChn;				/**< 选择回音消除的通道*/
	int Rev;							/**< 保留 */
} IMPAudioIChnParam;
 ...
 */
typedef struct {
   int threshold_low;       /** drc能力阈值**/
   int threshold_high;
   float ratio;             /** drc 比率 0.3~0.5 **/
   int eq_type;             /** 0:EQ_LINEAR 1:EQ_MEL_HTK,*/
   int freq_low;   int freq_high;
   int size;   float* EqTable;
} IMPAudioDrcEqConfig;
 ...
 * @param[in] audioDevId 音频输入设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[in] dgain 音频通道输入数字增益.

 * 范围[0 ~ 255],对应[-97dB ~ 30dB],步长0.5dB.
 ...
 * @attention aiGain的范围为[0 ~ 255],如果输入的值小于0,则aiGain的值将会\n
 * 被设置为0.如果值大于255,aiGain的值会被设置为255.

 */
 int IMP_AI_SetDigitalGain(int audioDevId, int aiChn, int dgain);

/**
 * @fn IMP_AI_GetDigitalGain(int audioDevId, int aiChn,int *dgain)

 * 获取音频输入数字增益.

 * @param[in] audioDevId 音频输入设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[out] dgain 音频通道输入数字增益属性指针
...
/**
 * @fn int IMP_AO_SetDigitalGain(int audioDevId, int aoChn, int dgain)

 * 设置音频输出增益.

 * @param[in] audioDevId 音频输出设备号.
 * @param[in] aoChn 音频输出通道号.
 * @param[in] dgain 音频输出增益,范围[0 ~ 0xFF],对应[-120dB ~ 7dB],步长0.5dB.
 ...
 * @attention aoGain的范围为[0 ~ 255],如果输入的值小于0,则dgain的值将会\n
 * 被设置为0.如果值大于255,dgain的值会被设置为255.

 */
 int IMP_AO_SetDigitalGain(int audioDevId, int aoChn, int dgain);

/**
 * @fn int IMP_AO_GetDigitalGain(int audioDevId, int aoChn, int *dgain)

 * 设置音频输出增益.

 * @param[in] audioDevId 音频输出设备号.
 * @param[in] aoChn 音频输出通道号.
 * @param[out] dgain 音频输出增益,范围[0 ~ 0xFF],对应[-120dB ~ 7dB],步长0.5dB.
...
/**
 * @fn int IMP_AI_DisableAlgo(void)
...
/**
 * @fn int IMP_AO_DisableAlgo(void)
...
/**
 * @fn int IMP_AI_EnableGetRaw(int audioDevId, int aiChn);
...
/**
 * @fn int IMP_AI_DisableGetRaw(int audioDevId, int aiChn);
 ...
 * @attentio 无.
 */
int IMP_AI_DisableGetRaw(int audioDevId, int aiChn);

/**
 * @fn int IMP_AI_GetFrameAndRaw(int audioDevId, int aiChn, IMPAudioFrame *frm, IMPAudioFrame *raw_frm, IMPBlock block);

 * 获取音频数据和Raw数据.
 * @param[in] audioDevId 音频设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[out] frm 音频帧结构体指针
 * @param[out] raw_frm Raw音频帧结构体指针
 * @param[in] block 阻塞/非阻塞标识
 ...
 * @attentio 获取Raw data 前先调用IMP_AI_EnableGetRaw使能.
 */
int IMP_AI_GetFrameAndRaw(int audioDevId, int aiChn, IMPAudioFrame *frm, IMPAudioFrame *raw_frm, IMPBlock block);

/**
 * @fn int IMP_AI_EnableLpf(IMPAudioIOAttr *attr)

 * 启用音频输入的低通滤波.

 * @param[in] attr 需要进行低通滤波的音频属性.
 ...
 * @remarks 无.
 */
int IMP_AI_EnableLpf(IMPAudioIOAttr *attr);

/**
 * @fn int IMP_AI_SetLpfCoFrequency(int cofrequency)

 * 设置音频输入的低通滤波器的截止频率.

 * @param[in] cofrequency 低通滤波的截止频率
 ...
 * @attention 开启低通滤波器之前先设置截至频率
 */
int IMP_AI_SetLpfCoFrequency(int cofrequency);

/**
 * @fn int IMP_AI_DisableLpf(void)
 ...
 * @attention 无.
 */
int IMP_AI_DisableLpf(void);

/**
 * @fn int IMP_AI_EnableDrcAndEq(IMPAudioIOAttr *attr, IMPAudioDrcEqConfig* drcEqConfig,int drc_enable, int eq_enable)
 *
 * 启用Ai动态范围压缩和均衡器功能
 *
 * @param[in] attr 需要进行的音频属性.
 * @param[in] drcEqConfig 动态范围压缩和均衡器的参数配置.
 * @param[in] drc_enable drc开关 1：使能 0：未使能
 * @param[in] eq_enable  eq开关  1：使能 0：未使能
 *
 * @retval 0 成功.
 * @retval 非0 失败.
 *
 *
 * @attention 只支持8k和16K采样率.
 */
int IMP_AI_EnableDrcAndEq(IMPAudioIOAttr *attr, IMPAudioDrcEqConfig* drcEqConfig,int drc_enable, int eq_enable);

/**
 * @fn int IMP_AI_DisableDrcAndEq(void)
 *
 * 禁用Ai动态范围压缩和均衡器功能.
 *
 * @param 无.
 ...
 * @attention 无.
 */
int IMP_AI_DisableDrcAndEq(void);

/**
 * @fn int IMP_Audio_Select_Codec(int audioDevId, int aiChn, int codec);

 * 音频内外部codec选择.
 * @param[in] audioDevId 音频设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[in] codec 音频内外部codec参数. SELECT_EXTERNAL：外部codec  SELECT_INTERNAL：内部codec
 ...
 * @attentio
	1、该函数应在IMP_AI_Enable 、IMP_AI_EnableChn 、IMP_AI_SetGain后调用。
	2、该函数不支持录放音取流途中动态切换。
	3、若不调用该函数，默认使用内部codec。
 */
int IMP_Audio_Select_Codec(int audioDevId, int aiChn, int codec);
