...
static void *sample_soft_photosensitive_thread(void *p)
{
	...
	//bThe statistical values of the (green/blue) component in the YUV image domain.
	...
	while (g_soft_ps_running) {
...
#endif
		//Obtain the exposure AE information:
		ret = IMP_ISP_Tuning_GetAeExprInfo(vinum, &ExpInfo);
		ret = IMP_ISP_Tuning_GetAeOnlyReadAttr(vinum, &aeOrAttr);
		if (ret == 0) {
			iso_buf = aeOrAttr.ExposureValue;
			luma = aeOrAttr.luma;
			luma_scence = aeOrAttr.luma_scence;
#ifdef SOFT_PHOTOSENSITIVE_DEBUG
			printf("-------------------AeExprInfo--------------------\n");
			printf("u32ExposureValue: %lld\n", aeOrAttr.ExposureValue);
			printf("u32AeAGain: %d\n", ExpInfo.AeAGain);
			printf("u32AeDGain: %d\n", ExpInfo.AeDGain);
			printf("--------------------AeScence---------------------\n");
			printf("Luma: %d, Luma Scence: %d\n", aeOrAttr.luma, aeOrAttr.luma_scence);
#endif
		} else {
			return NULL;
		}
		...
		//If the average brightness is less than 20, switch to night vision mode.
		if ((iso_buf > 200000) && ((luma < 50) || (luma_scence > 1000000))) {
			...
			if (night_count >= 6) {
				...
				//After switching to night vision, take the minimum value of 20 gb_gains as the reference value gb_gain_record for switching to daytime, where gb_gain is the G/B component of the Bayer pattern.
				for (i=0; i<20; i++) {
		...
		//Enter the daytime switching judgment condition when these three conditions are met:
