...
int SmolRTSP_RtpTransport_send_packet_ext(
    SmolRTSP_RtpTransport *self, SmolRTSP_RtpTimestamp ts, bool marker,
    U8Slice99 payload_header, U8Slice99 payload, uint8_t *extension_data, uint16_t extension_len) {
    assert(self);
    ...
    const SmolRTSP_RtpHeader header = {
        ...
        .csrc = NULL,
        .extension_profile = htons(0),
        .extension_payload_len = htons(extension_len),
        .extension_payload = extension_data,
    };
