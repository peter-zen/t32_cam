...
int SmolRTSP_NalTransport_send_packet_ext(
    ...
    if (nalu_size < max_packet_size) {
        ...
        return SmolRTSP_RtpTransport_send_packet_ext(
            self->transport, ts, marker,
            U8Slice99_new(header_buf, header_buf_size), 
            nalu.payload, extension_data, extension_len);
    }
...
int SmolRTSP_NalTransport_send_packet(
    SmolRTSP_NalTransport *self, SmolRTSP_RtpTimestamp ts,
    SmolRTSP_NalUnit nalu) {
    return SmolRTSP_NalTransport_send_packet_ext(
        self, ts, nalu, NULL, 0);
}
