...
`stillSize` 存储为分辨率枚举值（0~6），直接对应分辨率表的索引：

- 0 = 2M, 1 = 4M, 2 = 8M, 3 = 16M, 4 = 24M, 5 = 32M, 6 = 42M
- 默认值: 1 (4M)

### 属性管道交互

- **getter**: 枚举值 → 名称字符串（如 2 → "8M"）
- **setter**: 名称字符串 → 枚举值（如 "8M" → 2）
- **可选列表**: 当前仅 GC4653 sensor，支持全部 7 档
